...
export class WingGeometry extends THREE.BufferGeometry {
  constructor(is_main, side, span, sweep, dihedral, mount, washout, rootChord, tipChord, root_airfoil, tip_airfoil, nSeg, nAFseg, left_start, right_start, dy, control, same_as_root) {
    ...
    this.parameters = {
      ...
      mount: mount || 0.0,
      washout: washout || 0.0,
      rootChord: rootChord || 1.0,
      tipChord: tipChord || 1.0,
    ...
    generateWingGeometry(vertices, indices, {
      ...
      rootChord: this.parameters.rootChord,
      tipChord: this.parameters.tipChord,
...
function generateWingVerticesAndControlSurfaces(vertices, config) {
  ...
  for (let y = 0; y <= nSeg; y++) {
    ...
    let my_chord = rootChord + percent * (tipChord - rootChord);
